...
namespace algebra.class
set_option default_priority 0
local attribute [reducible] id

instance op_compatibility_of_op_left_compatible (op : α → β → β) (op' : α → α → α) [op_left_compatible op op'] : op_compatibility op op' op op :=
op_compatibility.of_pattern _ _ _ _

instance op_compatibility_of_op_right_compatible (op : α → β → α) (op' : β → β → β) [op_right_compatible op op'] : op_compatibility op op op op' :=
op_compatibility.of_pattern _ _ _ _

instance op_left_compatible_of_op_associative (op : α → α → α) [op_associative op] : op_left_compatible op op :=
op_left_compatible.of_pattern _ _

instance op_right_compatible_of_op_associative (op : α → α → α) [op_associative op] : op_right_compatible op op :=
op_right_compatible.of_pattern _ _

instance op_oppositive_of_op_symmetric (op : α → α → β) [op_symmetric op] : op_opposite op op :=
op_opposite.of_pattern _ _

instance op_symmetric_of_op_commutative (op : α → α → α) [op_commutative op] : op_symmetric op :=
op_symmetric.of_pattern _

instance fn_ct_homomorphism_of_fn_fixpoint (fn : α → α) (ct : α) [fn_fixpoint fn ct] : fn_ct_homomorphism fn ct ct :=
fn_ct_homomorphism.of_pattern _ _ _

instance fn_fn_homomorphism_of_fn_idempotent (fn : α → α) [fn_idempotent fn] : fn_fn_homomorphism fn fn id :=
fn_fn_homomorphism.of_pattern _ _ _

instance fn_fn_homomorphism_of_fn_fn_commutative (fn_1 : α → α) (fn_2 : α → α) [fn_fn_commutative fn_1 fn_2] : fn_fn_homomorphism fn_1 fn_2 fn_2 :=
fn_fn_homomorphism.of_pattern _ _ _

instance fn_fn_inverse_of_fn_involutive (fn : α → α) [fn_involutive fn] : fn_fn_inverse fn fn :=
fn_fn_inverse.of_pattern _ _

instance op_left_ct_homomorphism_of_op_left_fixpoint (op : α → β → α) (ct : α) [op_left_fixpoint op ct] : op_left_ct_homomorphism op ct ct :=
op_left_ct_homomorphism.of_pattern _ _ _

instance op_right_ct_homomorphism_of_op_right_fixpoint (op : α → β → β) (ct : β) [op_right_fixpoint op ct] : op_right_ct_homomorphism op ct ct :=
op_right_ct_homomorphism.of_pattern _ _ _

instance op_left_op_homomorphism_of_op_left_distributive (op_1 : α → β → α) (op_2 : α → α → α) [op_left_distributive op_1 op_2] : op_left_op_homomorphism op_1 op_2 op_2 :=
op_left_op_homomorphism.of_pattern _ _ _

instance op_right_op_homomorphism_of_op_right_distributive (op_1 : α → β → β) (op_2 : β → β → β) [op_right_distributive op_1 op_2] : op_right_op_homomorphism op_1 op_2 op_2 :=
op_right_op_homomorphism.of_pattern _ _ _
